...
import { createAuthorizedQueryResolver, nullResolver } from './resolvers';
...
/* 
  ...
  */
const forceAuthorizationOnFields = (fieldsGenerator, authorizationSchema) => {
  ...
  .reduce((fieldCarry, fieldName) => {
    if( typeof(securityResolvers) === 'undefined'
      ...
      return { 
        ...fieldCarry, 
        [fieldName]: {...originalFields[fieldName], resolve: nullResolver} 
      };
    };
    ...
    return { 
      ...
      [fieldName]: {
        ...
        resolve: createAuthorizedQueryResolver(
          authorizationSchema[fieldName].resolve,
...
const createAuthorizedGraphQLObjectType = (config, authorizationSchema) => {
  const newFields =
    typeof(config.fields) === 'function' 
    ? () => forceAuthorizationOnFields(() => config.fields, authorizationSchema)
    : forceAuthorizationOnFields(() => config.fields, authorizationSchema);
